...
class AndroidAutoLoopLoggerService : Service() {
    ...
    override fun onCreate() {
        ...
        workerThread = HandlerThread("AndroidAutoLoopLogger")
        workerThread?.start()
    ...
    private fun startLogger(durationMs: Long, intervalMs: Long) {
        ...
        transitions.clear()
        recentSamples.clear()
        filteredLogcatSeen.clear()
        lastLightEvidenceAt = 0L
        lastFullEvidenceAt = 0L
        lastCleanupAt = 0L
        lastFilteredLogcatAt = 0L
        fullEvidenceCount = 0
    ...
    private fun collectSample() {
        ...
        val tasks = parseAndroidAutoTasks(stackList)
        val visualState = classifyVisualState(tasks)
        val desiredPref = readDesiredAndroidAutoDisplayPref()
        val desiredProp = runShell("getprop persist.haval.android_auto.desired_display").trim()
        val usbSummary = summarizeUsb(runShell("dumpsys usb | head -80"))
        val servicePid = runShell("pidof com.ts.androidauto").trim().ifEmpty { "-" }
        val visualPid = runShell("pidof com.ts.androidauto.app").trim().ifEmpty { "-" }
        val sample = Sample(
            wallTime = wallTime(),
    ...
    private fun captureEvidence(reason: String, includeScreens: Boolean) {
        ...
        writeText(
            File(dir, "$prefix-window-androidauto.txt"),
            runShell("dumpsys window windows | grep -A28 -B6 'com.ts.androidauto.app/com.ts.androidauto.app.display.AapActivity' || true")
        )
        writeText(
        ...
        writeText(
            File(dir, "$prefix-surface-androidauto.txt"),
            runShell("dumpsys SurfaceFlinger | grep -A24 'SurfaceView - com.ts.androidauto.app/com.ts.androidauto.app.display.AapActivity' || true")
        )
        writeText(
            File(dir, "$prefix-services-androidauto.txt"),
            runShell("dumpsys activity services | grep -A28 -B6 'com.ts.androidauto' || true")
        )
        writeText(
            File(dir, "$prefix-settings-media-keys.txt"),
            runShell("settings get system bean_sw_custom_key1_config; settings get system bean_sw_custom_key2_config")
        )
        writeText(
            File(dir, "$prefix-logcat.txt"),
            runShell(filteredLogcatCommand(1200))
        )
    ...
    private fun parseAndroidAutoTasks(stackList: String): List<TaskSnapshot> {
        ...
        for (line in stackList.lines()) {
            ...
            if (packageName != ANDROID_AUTO_PACKAGE) continue
    ...
    private fun formatSample(sample: Sample): String {
        ...
        return "${sample.wallTime} elapsed=${sample.elapsedMs} visual=${sample.visualState} " +
            ...
            "usb=\"${sample.usbSummary}\" servicePid=${sample.servicePid} visualPid=${sample.visualPid} tasks=\"$taskSummary\""
    }

    private fun maybeCollectFilteredLogcat() {
        val now = SystemClock.elapsedRealtime()
        if (now - lastFilteredLogcatAt < FILTERED_LOGCAT_INTERVAL_MS) return
        lastFilteredLogcatAt = now

        val output = runShell(filteredLogcatCommand(260))
        val newLines = output.lines()
            .map { it.trimEnd() }
            .filter { it.isNotBlank() && filteredLogcatSeen.add(it) }
        if (newLines.isEmpty()) return

        appendLine("filtered-logcat.log", "${wallTime()} begin")
        for (line in newLines) {
            appendLine("filtered-logcat.log", line)
        }
        while (filteredLogcatSeen.size > MAX_FILTERED_LOGCAT_SEEN) {
            val iterator = filteredLogcatSeen.iterator()
    ...
    private fun filteredLogcatCommand(lines: Int): String {
        return "logcat -d -v time -t $lines | grep -Ei " +
            "'AndroidAuto|Android Auto|androidauto|AAP|AapActivity|LinkCommand|" +
            "DisplayAppLauncher|ServiceManager|InstrumentProjector2|" +
            "msgId=135|Cluster media|steering media|media input probe|MEDIA_|KEYCODE_MEDIA|" +
            "SYS_AVM|AVM|preview_status|backcamera|camera|CAR_HVAC|HVAC|beantechs.hvac|" +
            "requestVideoFocus|view_state|ActivityTaskManager|WindowManager' || true"
    }
    ...
    private fun readDesiredAndroidAutoDisplayPref(): Int {
        return try {
            App.getDeviceProtectedContext()
                .getSharedPreferences("haval_prefs", Context.MODE_PRIVATE)
                .getInt(PREF_DESIRED_ANDROID_AUTO_DISPLAY_ID, -1)
        } catch (e: Exception) {
            -1
        }
    }
    ...
    private fun createSessionDir(): File {
        val root = getExternalFilesDir("android-auto-loop")
            ?: File(filesDir, "android-auto-loop")
    ...
    private fun cleanupOldSessions(force: Boolean) {
        ...
        val roots = listOfNotNull(
            getExternalFilesDir("android-auto-loop"),
            File(filesDir, "android-auto-loop").takeIf { it.exists() }
    ...
    private fun writeSessionReadme() {
        ...
        writeText(
            File(dir, "README.txt"),
            """
            Android Auto loop logger session.
            ...
            Start:
            am startservice -n ${packageName}/.services.AndroidAutoLoopLoggerService -a $ACTION_START --el $EXTRA_DURATION_MS 604800000 --el $EXTRA_INTERVAL_MS 1500

            Stop:
            am startservice -n ${packageName}/.services.AndroidAutoLoopLoggerService -a $ACTION_STOP

            Manual dump:
            am startservice -n ${packageName}/.services.AndroidAutoLoopLoggerService -a $ACTION_DUMP

            Files:
            - state.log: sampled D0/D3 visual state, desired target, pids and task bounds
            - events.log: start/stop/transitions/captures
            - filtered-logcat.log: deduped AA/HVAC/AVM/media logs collected while sampling
            - *-services-androidauto.txt: Android Auto service/binder state
            - *-settings-media-keys.txt: native steering custom-key system settings
            """.trimIndent()
        )
    }
    ...
    private fun buildNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Android Auto loop logger ativo")
    ...
    companion object {
        private const val TAG = "AndroidAutoLoopLogger"
        private const val CHANNEL_ID = "AndroidAutoLoopLoggerChannel"
        ...
        const val ACTION_START = "br.com.redesurftank.havalshisuku.action.ANDROID_AUTO_LOOP_LOGGER_START"
        const val ACTION_STOP = "br.com.redesurftank.havalshisuku.action.ANDROID_AUTO_LOOP_LOGGER_STOP"
        const val ACTION_DUMP = "br.com.redesurftank.havalshisuku.action.ANDROID_AUTO_LOOP_LOGGER_DUMP"
        private const val ACTION_STOP_LEGACY = "br.com.redesurftank.havalshisuku.ANDROID_AUTO_LOOP_LOGGER_STOP"
        private const val ACTION_DUMP_LEGACY = "br.com.redesurftank.havalshisuku.ANDROID_AUTO_LOOP_LOGGER_DUMP"
        ...
        private const val FILTERED_LOGCAT_INTERVAL_MS = 5_000L
        private const val MAX_FILTERED_LOGCAT_SEEN = 1_200
