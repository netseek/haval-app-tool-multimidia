...
def apply_activity_patches(path):
    ...
    # D. onWindowFocusChanged: force hasFocus = true (p1 = 1)
    if CARPLAY_FOCUS_SENTINEL in content:
        print("  [SKIP] onWindowFocusChanged focus already forced")
    else:
        pattern = r"(\.method public onWindowFocusChanged\(Z\)V\s*\n\s*\.locals \d+\s*\n\s*\.param p1, \"hasFocus\"    # Z\s*\n)"
        replacement = r"\1    const/4 p1, 0x1\n\n    " + CARPLAY_FOCUS_SENTINEL + "\n"
        content, ok = patch_regex(content, pattern, replacement, "onWindowFocusChanged: force hasFocus to true")
        if ok:
            count += 1

    # E. showFragment: change .locals 4 to .locals 5 and insert equals patch
    if CARPLAY_SHOWFRAGMENT_SENTINEL in content:
        print("  [SKIP] showFragment equals check already patched")
    else:
        # Match showFragment method header and change .locals 4 to .locals 5
        pattern_locals = r"(\.method private showFragment\(Lcom/ts/carplay/app/ui/display/constants/DisplayConstants\$FragmentType;\)V\s*\n\s*\.locals )4"
        replacement_locals = r"\g<1>5"
        content, ok_locals = patch_regex(content, pattern_locals, replacement_locals, "showFragment: change locals count from 4 to 5")
        
        # Match the equals check block and replace it
        pattern_equals = (
            r"(iget-object v0, p0, Lcom/ts/carplay/app/ui/display/view/CarPlayDisplayActivity;->mCurFragmentType:Lcom/ts/carplay/app/ui/display/constants/DisplayConstants\$FragmentType;\s*\n\s*"
            r"if-ne p1, v0, :cond_1\s*\n"
            r".*?\s*return-void\s*\n\s*\n?\s*)"
            r"(.*?)"
            r"(:cond_1)"
        )
        
        replacement_equals = r"""\1# CARPLAY_SHOWFRAGMENT_EQUALS_PATCH
    # It matches. Let's see if we should trigger show()
    # 1. Check if mFragment is an instance of CarPlayDisplayFragment
    iget-object v0, p0, Lcom/ts/carplay/app/ui/display/view/CarPlayDisplayActivity;->mFragment:Lcom/ts/carplay/app/ui/display/view/BaseFragment;

    instance-of v1, v0, Lcom/ts/carplay/app/ui/display/view/CarPlayDisplayFragment;

    if-eqz v1, :cond_return_early

    check-cast v0, Lcom/ts/carplay/app/ui/display/view/CarPlayDisplayFragment;

    # 2. Check if mHasShown is false
    iget-boolean v1, v0, Lcom/ts/carplay/app/ui/display/view/CarPlayDisplayFragment;->mHasShown:Z

    if-nez v1, :cond_return_early

    # 3. Check if mDisplaySurface is not null
    iget-object v1, v0, Lcom/ts/carplay/app/ui/display/view/CarPlayDisplayFragment;->mDisplaySurface:Landroid/view/Surface;

    if-eqz v1, :cond_return_early

    # 4. Check if mDisplayPresenter is not null
    iget-object v2, p0, Lcom/ts/carplay/app/ui/display/view/CarPlayDisplayActivity;->mDisplayPresenter:Lcom/ts/carplay/app/ui/display/view/DisplayContract$Presenter;
    ...
    # Re-fetch surface and fragment because registers might be dirty
    iget-object v0, p0, Lcom/ts/carplay/app/ui/display/view/CarPlayDisplayActivity;->mFragment:Lcom/ts/carplay/app/ui/display/view/BaseFragment;
    ...
    # Call show
    invoke-interface {v2, v1, v3, v4}, Lcom/ts/carplay/app/ui/display/view/DisplayContract$Presenter;->show(Landroid/view/Surface;II)Z
...
\2\3"""
        ...
        content, ok_equals = patch_regex(content, pattern_equals, replacement_equals, "showFragment: inject equals check presenter show() re-trigger")
        if ok_equals:
            count += 1
...
def apply_fragment_patches(path):
    ...
    if ".field public mDisplaySurface:Landroid/view/Surface;" in content:
        print("  [SKIP] mDisplaySurface and mHasShown already public")
    else:
        content, ok1 = patch_direct(
            content,
            ".field private mDisplaySurface:Landroid/view/Surface;",
            ".field public mDisplaySurface:Landroid/view/Surface;",
            "CarPlayDisplayFragment: make mDisplaySurface public"
        )
        content, ok2 = patch_direct(
            content,
            ".field private volatile mHasShown:Z",
            ".field public volatile mHasShown:Z",
            "CarPlayDisplayFragment: make mHasShown public"
        )
        if ok1 or ok2:
            count += 1
...
def apply_fragment_callback_patches(path):
    ...
    # If presenter is null, we can never call show, so skip it immediately!
